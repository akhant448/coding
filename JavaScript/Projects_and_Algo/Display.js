...
// SLL1.front() => null
    ...
    display() {
        var listStr = "" //empty string that will contain value
        if (this.head == null) {
            return listStr;
        }

        listStr += this.head.value; //this linked the first value to the empty string
        var another = this.head.next; //assign the variable another to the next value
        while (another != null) {
            listStr += "," + another.value;
            another = another.next; //move the another to the next node
        }
        return listStr;
    }
}
